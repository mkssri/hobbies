...
def get_volunteer_satisfaction_score(assignments, volunteers, num_tasks):
    """
...
def load_tasks(csv_filename):
    tasks = {}  # Maps task_id -> Task
...
def load_volunteers(csv_filename, tasks):
    volunteers = []
    with open(csv_filename, "r") as csv_file:
        csv_reader = csv.reader(csv_file)
        try:
            for parsed_line in csv_reader:
                ...
                for task_id_string in interested_tasks.split():
                    volunteer.add_interested_task(tasks.get(int(task_id_string)))
